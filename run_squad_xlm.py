...
# this tools used for zh evaluation
from transformers.data.metrics.squad_metrics import compute_predictions_logits

from src.utils_mlqa import (read_squad_examples, read_mlqa_examples,
...
from src.utils_squad_evaluate import EVAL_OPTS, main as evaluate_on_squad
from src.utils_mlqa_evaluate import MLQA_EVAL_OPTS, evaluate_on_mlqa
...
def train(args, train_dataset, model, tokenizer, teacher=None):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            # print('batch start position', batch[3])
            ...
            if args.model_type in ['xlnet', 'xlm']:
                inputs.update({'langs': batch[1]})
                if isinstance(model, XLMForQuestionAnswering):
                    inputs.update({'cls_index': batch[6],
                                   'p_mask': batch[7]})

            outputs = model(**inputs)
            loss, *remains = outputs

            # Distillation loss
            if teacher is not None:
                start_logits_stu, end_logits_stu = remains
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.evaluate_step == 0:
                    # Log metrics
...
def evaluate(args, model, tokenizer, prefix=""):
    eval_dataset = args.predict_dataset
    ...
    if args.model_type in ['xlnet']:
        ...
        write_predictions(examples, features, all_results, args.n_best_size,
                          ...
                          args.version_2_with_negative, args.null_score_diff_threshold, tokenizer, args.predict_lang)

    # Evaluate with the official SQuAD script or official MLQA script
    if eval_dataset == 'SQuAD':
        evaluate_options = EVAL_OPTS(data_file=args.predict_file,
                                     pred_file=output_prediction_file,
                                     na_prob_file=output_null_log_odds_file)
        results = evaluate_on_squad(evaluate_options)
    elif eval_dataset == 'MLQA':
        evaluate_options = MLQA_EVAL_OPTS(dataset_file=args.predict_file,
                                          prediction_file=output_prediction_file,
                                          answer_language=args.predict_lang,
                                          na_prob_file=output_null_log_odds_file)
        results = evaluate_on_mlqa(evaluate_options)
    else:
        raise ValueError(
            'Evaluate function for {} dataset is not supported'.format(eval_dataset))
...
def load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False):
    ...
    dataset = args.predict_dataset if evaluate else args.train_dataset
    input_file = args.predict_file if evaluate else args.train_file
    input_lang = args.predict_lang if evaluate else args.train_lang
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache and not output_examples:
        ...
        if dataset == 'SQuAD':
            ...
            examples = read_mlqa_examples(input_file=input_file,
                                          is_training=not evaluate,
                                          version_2_with_negative=args.version_2_with_negative)
        else:
            raise ValueError('Don\'t support {} dataset'.format(dataset))
    ...
    all_lang_ids = torch.tensor(
        [f.lang_ids for f in features], dtype=torch.long)
    ...
    if evaluate:
        ...
        dataset = TensorDataset(all_input_ids, all_lang_ids, all_input_mask, all_segment_ids,
...
def main():
    ...
    # Required parameters
    parser.add_argument("--train_file", default="data/train-v1.1.json", type=str, required=True,
                        help="SQuAD json for training. E.g., train-v1.1.json")
    parser.add_argument("--train_lang", default="en", type=str, required=True,
                        help="Specify the input language for using language embedding in XLM")
    parser.add_argument("--train_dataset", default="SQuAD", type=str, required=True,
                        help="Specify the input dataset for differnet reader (SQuAD, MLQA)")
    parser.add_argument("--predict_file", default=None, type=str, required=True,
                        help="SQuAD json for predictions. E.g., dev-v1.1.json or test-v1.1.json")
    parser.add_argument("--predict_lang", default="en", type=str, required=True,
                        help="Specify the input language for using language embedding in XLM")
    parser.add_argument("--predict_dataset", default="MLQA", type=str, required=True,
                        help="Specify the input dataset for differnet reader (SQuAD, MLQA)")
    parser.add_argument("--model_type", default="xlm", type=str, required=True,
                        help="Model type selected in the list: " + ", ".join(MODEL_CLASSES.keys()))
    parser.add_argument("--model_name_or_path", default="xlm-mlm-tlm-xnli15-1024", type=str, required=True,
    ...
    parser.add_argument("--total_steps", default=0, type=int,
                        help="Linear warmup scheduler total step.")
    ...
    parser.add_argument('--evaluate_step', type=int, default=3000,
                        help="Do Evaluate during training every X updates steps.")
    parser.add_argument("--model_checkpoint_dir", default="", type=str,
                        help="Specify the model checkpoint to be evaluated")
    ...
    tokenizer = tokenizer_class.from_pretrained(
        args.tokenizer_name if args.tokenizer_name else args.model_name_or_path, do_lowercase_and_remove_accent=args.do_lower_case)
    ...
    if args.teacher_type is not None:
        ...
        teacher = teacher_model_class.from_pretrained(
            args.teacher_name_or_path, config=teacher_config)
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        tokenizer = tokenizer_class.from_pretrained(
            args.output_dir, do_lowercase_and_remove_accent=args.do_lower_case)
        model.to(args.device)

    # Evaluation - we can ask to evaluate all the checkpoints (sub-directories) in a directory
    results = {}
    if args.do_eval and args.local_rank in [-1, 0]:
        if args.model_checkpoint_dir:
            checkpoints = [args.model_checkpoint_dir]
        else:
        ...
        logger.info("Language: {}".format(args.predict_lang))
